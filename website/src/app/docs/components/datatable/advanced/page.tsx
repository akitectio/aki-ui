  ...
  const mockEnterpriseApi = {
    generateEmployeeData: (count: number = 50) => {
      ...
      return Array.from({ length: count }, (_, i) => ({
        ...
        salary: 80000 + (i * 2500) + Math.floor(Math.random() * 30000),
        ...
        performance: Math.max(60, Math.min(100, 75 + Math.floor(Math.random() * 30) - 15)),
        skillsCount: Math.floor(Math.random() * 6) + 2,
        primarySkill: skills[i % skills.length],
        yearsExp: Math.min(15, Math.floor((Date.now() - new Date(2018 + Math.floor(i / 12), 0, 1).getTime()) / (365 * 24 * 60 * 60 * 1000)) + Math.floor(Math.random() * 5)),
